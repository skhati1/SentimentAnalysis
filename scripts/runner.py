...
def loadDataset(n):
	inputData = pd.read_csv('../data/training_1.csv')
	...
	inputData['Sentiment'] = inputData['Sentiment'].map( {4:1, 0:0} )
...
#now we can feed these vectors to a decision tree classifier for processing using Keras
#derivedModel = DecisionTreeClassifier(random_state=0, )
#derivedModel.fit(train_vectors, y_train)
#accuracy = derivedModel.score(test_vectors, y_test)
#print('Accuracy achieved was', accuracy)


#accuracy = derivedModel.score(test_vectors, y_tes)
#print('Accuracy achieved was', accuracy)
